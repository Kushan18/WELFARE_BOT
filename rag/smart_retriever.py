...
def smart_retrieve(query, user_state=None):
    ...
    logger.warning("smart_retrieve: ChromaDB empty, falling back to live (no URLs)")
    # No cached results; we cannot perform a generic live search per user request.
    return [], "none"
...
def detect_state(query, user_state=None):
    if user_state:
        return user_state.lower()
    q = query.lower()
    if "telangana" in q: return "telangana"
    if "andhra" in q or "ap" in q: return "andhra pradesh"
    if "central" in q or "pm " in q: return "central"
    return None
